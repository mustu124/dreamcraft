// Color/finish preference offered on every product page except Customization.
// This is a customer preference for the artisan — it never affects price or
// stock, so it deliberately isn't modeled as a real product_variant. The
// choice is carried on the cart item as a plain label and passed through to
// the order as its own field (order_items.color_label).
...
// The two terminal (non-shade) finish options — no sub-picker needed.
export type ColorGroup = "dark" | "pastel" | "blocked" | "marble";
